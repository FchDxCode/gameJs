...
function update() {
  bg.y += 5;
  bg2.y += 5;
  if (bg.y >= 1000) { //diubah untuk meng loop background nya
    bg.y = -1000;
  } else if (bg2.y >= 1000) {
    bg2.y = -1000;
  }
  ...
  for (var b = 0; b < enemies.children.length; b++) {
    ...
    if (
      ...
      ship.y = 1000 + 34;
      Tween.get(ship).to({ y: 700 }, 500); //diubah, ini mengubah posisi awal saat ship terkena musuh
      SoundJS.play("explo");
    }
  }
...
function addEnemy() {
  ...
  a.x = Math.floor(Math.random() * (350 - 50));
  a.y = -50;
  enemies.addChild(a);
  stage.update();
}
function shoot() {
  ...
  a.x = ship.x + 13; //diubah nilai awal 13
  a.y = ship.y - 20; //diubah nilai awal 20
  bullets.addChild(a);
  stage.update();
  SoundJS.play("shot");
}
...
function addGameView() {
  ship.x = centerX - 1.5;
  ship.y = 1000 + 34; //diubah nilai awal 1000 + 34 ini  untuk mengedit seberapa dekat tembakan dengan main character nya (atas bawah)
  for (var a = 0; a < 3; a++) {
    var b = new Bitmap(lImg);
    b.x = 375 + 50 * a; //diubah nilai awal 248 + 25 ini untuk mengedit samping kanan kiri tembakan main character nya
    b.y = 860; //untuk memindahkan posisi hati/nyawa
    lives.addChild(b);
    stage.update();
  }
  score = new Text("0", "bold 30px Courier New", "#FFFFFF");
  score.maxWidth = 1000;
  score.x = 10; //diubah untuk memindahkan posisi kanan kiri score
  score.y = 900; //diubah untuk memnindahkan posisi atas bawah score
  bg2.y = -1000; 
  stage.addChild(bg, bg2, ship, enemies, bullets, lives, score);
  Tween.get(ship).to({ y: 700 }, 3e3).call(startGame); //y: 700 diubah untuk mengatur atas bawah ship
}
function loadGfx(a) {
  if ((a.target.name = "bg")) {
    bg = new Bitmap(bgImg);
...
function Main() {
  ...
  SoundJS.addBatch([
    { name: "boss", src: "boss.mp3", instances: 1 }, //diubah (kalo mau diubah)
